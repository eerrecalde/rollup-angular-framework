...
describe('Directive_Ctp_Paginator', () => {
	let compile;
	let rootScope;
	let scope;
	let itemsMock;
	...
	const getProcessedObject = function (i, itemsPP, currentP) {
		scope.pobj = {};
		scope.items = i;
	...
	it(`When sending no items, it should return an object with empty arrays for items,
		paginatedItems, and pagesArray`, () => {
		currentPage = 0;
		itemsPerPage = 2;
		items = [];
	...
	it('When sending no currentPage, it should default to 0', () => {
		currentPage = null;
		itemsPerPage = 2;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
	...
	it('When sending no itemsPerPage, it should default to items.length', () => {
		currentPage = 0;
		itemsPerPage = null;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
	...
	it('When sending 2 as itemsPerPage, it should return an array of 2 element in the paginatedItems property', () => {
		currentPage = 0;
		itemsPerPage = 2;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
	...
	it('When sending 2 as itemsPerPage, it should match snapshot', () => {
		currentPage = 0;
		itemsPerPage = 2;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
		...
		const data2 = data.paginatedItems.map(el => {
	...
	it(`When sending 1 as current page, and 2 as itemsPerPage,
		getItemFrom should return 3, and getItemTo should return 4`, () => {
		currentPage = 1;
		itemsPerPage = 2;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
	...
	it('When paginate to 1, currentPage should be updated to 1', () => {
		currentPage = 0;
		itemsPerPage = 1;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
	...
	it('When using setItemsPerPage, itemsPerPage should update', () => {
		currentPage = 0;
		itemsPerPage = 2;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
	...
	it(`When currentPage == 2, if the amount of pages gets less than 2,
		it should readjust currentPage to latest possible`, () => {
		currentPage = 3;
		itemsPerPage = 1;
		items = itemsMock;
		data = getProcessedObject(items, itemsPerPage, currentPage);
